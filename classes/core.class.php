<?php
	...
	class Core{
		...
		public function setTitle($title){
			...
			$tplSite->assign("__SECTION_TITLE__", $title);
		}
		...
		public function setMetaDescription($description){
			...
			$tplSection->assign("__META_DESCRIPTION__",$description);
		}
		...
		public function getCopyFromFile($file = ''){
			
			$tags = array('[h1]','[h2]','[h3]','[h3-complementary]','[keywords]','[main-content]');
			$copy = array();
			...
			if(file_exists($file)){
				...
				if($filem){
					...
					while (($line = fgets($filem, filesize($file))) !== false) {
						
						foreach($tags as $tag){
							
							if(strpos($line, $tag) !== false){
								
								$tag_content = explode($tag, $line);
								$copy[preg_replace("/(\[|\])/",'',$tag)] = trim($tag_content[1]);
								break;
